...
# Define BlogLike first (needed for Blog.column_property)
class BlogLike(Base):
...
# Define BlogComment before Blog (for column_property)
...
# Define Blog last (after its dependencies)
class Blog(Base):
    ...
    like_count = column_property(
        ...
        ).correlate_except(BlogLike).scalar_subquery()
    )
    ...
    comment_count = column_property(
        ...
        ).correlate_except(BlogComment).scalar_subquery()
    )
