...
DATABASE_URL = "postgresql+asyncpg://api:api@localhost:5432/school_info"
...
async def insert_batch_data(rows: List[List[Any]]) -> Tuple[int, int, int]:
    ...
    async with async_session() as session:
        for row in rows:
            try:
                stmt = select(SchoolInfo).where(SchoolInfo.id == row[0])
...
async def main(json_directory: str = "Info") -> None:
    print("=" * 60)
    ...
    inserted, updated, skipped = await insert_batch_data(store_data)
...
if __name__ == "__main__":
    directory = sys.argv[1] if len(sys.argv) > 1 else "Info"
