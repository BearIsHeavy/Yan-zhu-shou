...
| Volume | Purpose | Path |
|--------|---------|------|
| `postgres_data` | PostgreSQL database | `/var/lib/postgresql/data` |
...
- `uploads_data`: User-uploaded content (blog posts, user bios)
...
│   │   └── blog_<id>_<uuid>.md
│   └── ...
└── bios/           # User self-introduction files
    ├── <user_id>/
    │   └── <uuid>.md
    └── ...
```
...
# View blog files
docker-compose exec app ls -la /app/uploads/blogs/

# View bio files
docker-compose exec app ls -la /app/uploads/bios/

# Backup uploads
docker-compose exec app tar -czf /tmp/uploads_backup.tar.gz /app/uploads/
docker-compose cp app:/tmp/uploads_backup.tar.gz ./backups/

# Restore uploads (from backup)
docker-compose cp ./backups/uploads_backup.tar.gz app:/tmp/
docker-compose exec app tar -xzf /tmp/uploads_backup.tar.gz -C /
```
